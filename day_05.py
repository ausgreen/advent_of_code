...
line, with the current instruction marked in parentheses.
...
#thoughts
1. convert input to list of integers
2. start at element 0
3. grab value, increment list[index], counter, current index
4. if current index > len(list), exit else repeat 3 & 4
'''
...
def day_05(input):
    # 1. convert input to list of integers
    command_list = convert_input_to_list(input)

    # 2. start at element zero
    step_counter = 0
    index = 0
    # 3. grab value, increment list[index], counter, current index
    # 4. if current index > len(list), exit else repeat 3 & 4
